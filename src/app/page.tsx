...
export default function Home() {
  ...
  const { mutateAsync: confirmarMutation, isPending } = api.wedding.confirmar.useMutation({
    ...
    onError: (error) => {
      ...
      setMessage('Hubo un error al enviar tu confirmación. Por favor, inténtalo de nuevo más tarde.');
    }
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // onError handler above already sets a user message
    } finally {
      ...
      <div className="max-w-md mx-auto bg-white rounded-2xl shadow-lg overflow-hidden">
        <div className="px-4 py-6 sm:px-6 sm:py-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
            ...
            >
              {isSubmitting || isPending ? 'Enviando...' : 'Confirmar Asistencia'}
            </button>
